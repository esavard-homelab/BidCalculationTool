...
/** The selected vehicle type (Common or Luxury) */
const vehicleType = ref<string>('Common')
/** Available vehicle types loaded from the API */
const vehicleTypes = ref<VehicleTypeOption[]>([])
/** The calculated bid response from the API */
const calculation = ref<BidCalculationResponse | null>(null)
/** Validation error message for the price input */
...
/**
 * Loads available vehicle types from the backend API.
 * Sets loading state and handles errors appropriately.
 *
 * @throws Sets error state if the API request fails
 */
...
/**
 * Calculates the bid for the current vehicle price and type.
 * Performs validation, resets errors, and calls the backend API.
 * Updates the calculation result or sets appropriate error messages.
 *
 * @remarks
 * - Resets all error states before calculation
 * - Validates vehicle price using VehiclePriceValidator
 * - Only proceeds if price is a valid number
 * - Updates calculation state with API response or error
 */
...
/**
 * Component lifecycle hook that initializes the component.
 * Loads vehicle types when the component is mounted.
 */
onMounted(async () => {
  loading.value = true
